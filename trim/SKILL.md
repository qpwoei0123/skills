...
metadata:
  version: 0.3.0
description: 동작을 유지하면서 현재 diff의 코드량, 중복, 불필요한 복잡도를 줄이는 스킬. "코드 줄여줘", "단순하게 해줘", "다듬어줘", "정리해줘", "/trim" 등 작성한 변경분을 간결하게 만들자는 말이 나오면 사용한다.
---
...
diff가 크면 Gemini CLI가 있을 때 보조 분석을 맡길 수 있고, 결과는 참고용이다.
...
1. baseline을 기록한다.
   - `git diff --stat`
   - `git diff > /tmp/pre-trim.patch`로 적용 전 상태를 스냅샷한다. 이 스냅샷이 없으면 trim을 시작하지 않는다.
   - 가능한 검증 명령 결과
2. 낮은 위험도의 단순화만 적용한다.
3. 같은 검증 명령을 다시 실행한다.
4. `git diff --stat` 전후를 비교한다.
5. 실패하면 원인을 파악해 고치거나, 되돌린다. 되돌릴 때는 `git checkout -- <files>` 후 `git apply /tmp/pre-trim.patch`로 스냅샷 시점을 복원한다. 기존 사용자 변경은 임의로 revert하지 않는다.
