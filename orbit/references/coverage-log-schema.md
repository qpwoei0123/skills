...
**저장 위치:**
- `~/.orbit/<group>/<project>/<VIEW>.json` (view별 독립 파일)
- `~/.orbit/<group>/<project>/result.json` (마지막 실행 상세)
- 환경변수 `REPO_ORBIT_HOME`을 설정하면 기본 경로(`~/.orbit`)를 오버라이드한다.
- 첫 실행 시 디렉토리가 없으면 자동 생성한다. view 파일이 없으면 아래 초기값으로 생성한다.

**인증 설정:**
- `~/.orbit/auth.json`에 토큰을 저장한다. 스킬 폴더 안에 두지 않는다.
...
`~/.orbit/<group>/<project>/BUILD.json` 예시:
