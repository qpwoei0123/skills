...
{[변경 리뷰어 🕵️]/[커버리지 리뷰어 🧑‍💻]/[위험 리뷰어 👮] 중 관련 있는 1~2명만 쓴다. 리드 리뷰어는 [리드 리뷰어 🧑‍🏫]로 표기한다.}
...
- 본문에서 리뷰어를 가리킬 때는 역할 이모지를 붙인 태그 형식으로 쓴다: `[리드 리뷰어 🧑‍🏫]`, `[변경 리뷰어 🕵️]`, `[커버리지 리뷰어 🧑‍💻]`, `[위험 리뷰어 👮]`. 같은 문단에 같은 리뷰어가 여러 번 나오면 두 번째부터는 태그 없이 짧게 가리켜도 된다.
