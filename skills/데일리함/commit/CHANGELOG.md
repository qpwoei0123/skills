...
- 호출명·목록 표시명을 `commit`에서 `.⚡commit`으로 변경하고 닷 스킬 브랜드 적용
- 스킬 간 참조·호출 예시·검증 경로를 동기화하고 저장 폴더는 `commit` 사용
- PR/MR 준비·제출을 `.⚡ship`으로 연결하고 무관한 변경을 남긴 상태의 커밋 범위를 명확히 함

## 1.0.0

### Changed
- 기본 호출을 커밋 실행으로 변경하고 계획·메시지만 요청한 경우에는 제안으로 제한
- 진행 중인 작업과 후속 정정의 승인을 완료까지 유지하고 고정 승인·보고 템플릿 제거
- 의미 단위 분할, 사용자 stage 상태, 저장소 관례와 hook 보호를 유지하며 검증 결과 재사용

## 0.6.2

### Changed
- 분석·분할·메시지·실행 규칙의 반복 설명을 줄이고 실제 커밋 순서 중심으로 경량화
- 커밋과 PR/MR을 함께 요청하면 mr이 전체 흐름을 소유하고 commit은 커밋 단계만 담당하도록 연계 계약 명시
