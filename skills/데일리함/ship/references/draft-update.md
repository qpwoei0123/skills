...
push를 포함한 갱신은 최종 로컬 HEAD를 기준으로, 원격 본문만 갱신하는 요청은 해당 Draft의 원격 head를 기준으로 본문을 준비한다. 후자에는 아직 push하지 않은 로컬 커밋을 설명에 포함하지 않는다.
...
2. push가 요청 범위에 포함되면 CLI 인증을 사용할 수 있는 HTTPS 경로로 확인한 저장소의 현재 branch만 push한다. 거절되면 force push하지 않는다. 원격 본문만 갱신하는 요청에서는 push를 생략한다.
3. 같은 ID·open·Draft·source·target과 remote head SHA를 확인한다. push했다면 SHA는 push한 로컬 HEAD와 같아야 하고, 본문만 갱신한다면 snapshot의 remote head와 같아야 한다.
4. 원격 본문이 snapshot과 다르면 다른 사람의 수정을 덮지 않고 실제 완료한 단계까지만 보고한다.
