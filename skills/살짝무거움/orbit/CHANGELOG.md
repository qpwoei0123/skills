...
- 깊게 들어가는 도구를 `🌈`로 구분하는 닷 접두사 적용

- 호출명·목록 표시명을 `won-orbit`에서 `.🌈orbit`으로 변경하고 닷 스킬 브랜드 적용
- 스킬 간 참조·호출 예시·검증 경로를 동기화하고 저장 폴더는 `orbit` 사용

## 3.0.0

### Changed

- `orbit`의 폴더명·호출명·목록 표시명을 `won-orbit`으로 변경
...
- 기본 감사와 명시적 이슈 발행을 구분하고 기존 작업·자동화의 발행 권한은 유지
- 고정 3인·반박 라운드·재심 절차를 필요한 범위의 직접 조사·분담·근거 확인으로 변경
- 관점별 도구·폴더·파일 부재 기준을 실제 계약·영향·반례 중심으로 수정
- 분석 SHA를 파일·diff·메모리에 통일하고 dry-run의 지속 기록, 부분 조사와 발행 실패 누락을 방지하는 계약으로 정리
- 본문을 orbit/v2.4로 축약하고 기존 fingerprint·legacy alias·closed·suppressed 처리와 publisher CLI를 유지
