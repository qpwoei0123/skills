---
name: won-code-visualizer
license: Apache-2.0
metadata:
  version: 2.0.0
description: (v2.0.0) 대화·코드·diff의 구조, 호출, 데이터, 상태, 전후 변화를 근거가 연결된 단일 HTML 설명서로 만든다. "방금 설명을 HTML로 보여줘", "이 코드 흐름을 한눈에 보여줘", "diff 전후를 시각화해줘", "$won-code-visualizer" 요청에 사용한다. 제품 UI 구현, 결함 리뷰, 재설계 제안, Figma·슬라이드·이미지 제작은 각 전용 스킬이 맡는다.
---
