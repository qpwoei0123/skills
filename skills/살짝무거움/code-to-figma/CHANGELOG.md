...
- 호출명·목록 표시명을 `won-code-to-figma`에서 `.🌈code-to-figma`으로 변경하고 닷 스킬 브랜드 적용
- 스킬 간 참조·호출 예시·검증 경로를 동기화하고 저장 폴더는 `code-to-figma` 사용

## 2.0.0

### Changed

- `code-to-figma`의 폴더명·호출명·목록 표시명을 `won-code-to-figma`로 변경
...
- 고정 네 칸 확인과 화면 수에 따른 모드 전환을 제거하고 맥락·편집 요구·현재 capability로 실행
- 명시적인 새 파일 생성 요청을 지원하고 선택적 DS 탐색 실패의 기본 fallback을 정리
- capture 드라이버 설명을 실제 DOM-to-design 제출과 일치시키고 제출·생성 완료·DS 연결을 구분
- 읽기 성공과 편집 권한을 구분하며 부분 성공·중복 방지·이번 실행의 임시 결과만 정리하는 기준 유지
