...
```text
$won-soul-extractor 이 글을 내 샘플의 말투로 다듬어줘.
$won-soul-extractor 이 샘플에서 문체 프로필을 만들어 저장해줘.
$won-soul-extractor 이 초안의 문체 일치도를 채점해줘.
```
...
```bash
python3 scripts/validate_skills.py --skill won-soul-extractor
python3 -m unittest discover -s skills/살짝무거움/won-soul-extractor/scripts -p 'test_*.py'
```
