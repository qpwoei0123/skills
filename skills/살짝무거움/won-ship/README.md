...
현재 작업을 다듬고 커밋한 뒤 새 Draft PR/MR을 만들거나 기존 Draft 본문을 갱신합니다. Codex 표시 이름은 **won-ship**입니다.

## Quick Start

```text
$won-ship
$won-ship 수정하지 말고 출항 계획만 보여줘.
$won-ship 기존 Draft에 이번 작업을 이어서 반영해줘.
```

기본 호출은 `good → commit → Draft 생성/갱신`을 완료합니다. 계획 요청이면 읽기 전용으로 제안합니다. `/won-ship`, `--go`, `-go`도 지원하며, 명시된 계획·범위 제한이 우선합니다.
...
```bash
python3 scripts/validate_skills.py --skill won-ship
```
