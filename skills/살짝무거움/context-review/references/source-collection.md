...
이미 인증된 `gh`·`glab`이나 구조화된 API·connector를 우선한다. 한 명령의 요약 출력이 전체 자료라고 가정하지 않는다.
...
확인한 revision과 주요 행동 묶음, discussion·연결 자료의 범위를 밝힌다. 읽지 못한 중요한 파일이나 자료가 어떤 판단을 제한하는지 적는다.

큰 diff에서는 `확인한 의미 변경 파일 X/Y · 접은 기계적 파일 N · 확인 불가 N`으로 범위를 보완할 수 있다. 매핑했다는 사실을 정독·검증 완료로 표현하지 않는다. 작은 변경에는 별도 수치 보고를 강제하지 않는다.
