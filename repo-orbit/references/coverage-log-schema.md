...
triage-rules.md의 재검토 트리거와 view-rotation.md의 편중 경고가 이 파일을 기준으로 작동한다.
...
| `forced` | bool | `--focus-view` 강제 지정 여부 |
...
## view-rotation 편중 경고 계산 기준

| 경고 조건 | 계산 방법 |
|----------|----------|
| 특정 view 40% 이상 | 최근 30개 entry 중 해당 view_id 비율 |
| 특정 view 14일 이상 미사용 | run_at 최신값 기준으로 14일 초과 여부 |
| 7회 연속 동일 view | 최근 7개 entry의 view_id가 모두 같은 경우 |
