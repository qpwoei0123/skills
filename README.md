...
└── skills/                         # 정식 스킬 (사용 무게 기준 카테고리)
    ...
    └── 살짝무거움/                 # 멀티스텝 워크플로
        ...
        ├── ship/                   # 새 Draft 생성·기존 Draft 갱신까지 잇는 출항 워크플로
...
- `orbit 🪐`: 요청한 관점으로 레포를 분석하고, 발행 요청이 있으면 근거가 확인된 기술 이슈를 생성·갱신하는 스킬
- `soul-extractor`: 문체 지문을 추출해 글을 작성·변환하고 요청하면 일치도를 점검하는 스킬
- `code-to-figma`: 구현된 웹 화면을 요청한 편집 구조와 디자인 시스템 연결에 맞춰 Figma로 옮기는 스킬
- `ship`: 현재 작업을 다듬고 주석·검증·커밋한 뒤 새 Draft를 만들거나 기존 Draft 본문을 갱신하는 출항 워크플로
- `context-review`: 큰 PR/MR과 관련 자료에서 사람이 답해야 할 고맥락 리뷰 질문만 근거와 함께 추리는 스킬
- `code-visualizer`: 코드·diff의 아키텍처와 실행·데이터·상태 흐름을 근거가 연결된 단일 HTML로 보여주는 스킬
...
$orbit . --view SAFE --publish
$code-to-figma 현재 화면들을 새 Figma 파일에 편집 가능한 레이어로 옮겨줘.
$soul-extractor 이 글을 내 샘플의 문체로 다듬어줘.
```
...
`orbit`은 2.0, `context-review`·`code-visualizer`·`code-to-figma`·`soul-extractor`는 1.0으로 갱신했습니다. 근거·완료 조건은 유지하고 고정 인원·질문 수·화면 수별 모드·샘플 수·필수 점수표를 줄였습니다.

기본 `$orbit <repo>`는 분석입니다. 이슈 발행 자동화는 `--publish` 또는 발행 의도를 명시합니다. 기존 작업의 발행 권한은 유지하고 `--dry-run`은 payload만 준비하며 지속 메모리를 변경하지 않습니다. Codex UI 기본 프롬프트에는 발행 요청이 들어 있습니다. 본문은 `orbit/v2.4`를 사용하고 기존 fingerprint와 닫힌 이슈 처리 계약은 유지합니다.

Figma 변환은 화면 수와 관계없이 요청한 편집·DS 기준을 따릅니다. `soul-extractor`는 글을 먼저 완성하고 프로필 저장과 채점은 요청할 때 수행합니다. 각 스킬의 README에 실행 예시와 변경 이력이 있습니다.
